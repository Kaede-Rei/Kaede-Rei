# 👋 Hi, I'm Kaede-Rei

💻 **Embedded Software Engineer** | 🤖 **Robotics Developer** | 🧠 **Embodied AI Explorer**  

专注于 **嵌入式系统 / 六轴机械臂 / ROS / 具身智能 (IL + RL)**  
擅长从「底层驱动 + 控制算法 + 人机交互」的机器人机械臂电控系统开发。

---

## 🧩 Tech Stack

### 🔹 Embedded & Firmware
![C](https://img.shields.io/badge/-C-00599C?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/-C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![RTOS](https://img.shields.io/badge/-RTOS-E7352C?style=flat-square)
![STM32](https://img.shields.io/badge/-STM32-03234B?style=flat-square)
![ESP32](https://img.shields.io/badge/-ESP32-E7352C?style=flat-square)
![Arduino](https://img.shields.io/badge/-Arduino-00979D?style=flat-square)
![Renesas](https://img.shields.io/badge/-Renesas-CC0000?style=flat-square)

### 🔹 Robotics & Control
![C++](https://img.shields.io/badge/-C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python)
![ROS](https://img.shields.io/badge/-ROS1/2-22314E?style=flat-square&logo=ros)
![MoveIt](https://img.shields.io/badge/-MoveIt-FF6F00?style=flat-square)
![OpenCV](https://img.shields.io/badge/-OpenCV-5C3EE8?style=flat-square&logo=opencv)
![LeRobot](https://img.shields.io/badge/-LeRobot-000000?style=flat-square)

### 🔹 HMI & Tools
![LVGL](https://img.shields.io/badge/-LVGL-2C3E50?style=flat-square)
![Qt](https://img.shields.io/badge/-Qt-41CD52?style=flat-square&logo=qt)
![Rust](https://img.shields.io/badge/-Rust-000000?style=flat-square&logo=rust)

### 🔹 Hardware
PCB Design · 电路调试 · 多机通信 · 电机驱动 · 嵌入式视觉

---

## 📌 About Me

- 🤖 专注：机械臂底层控制 / ROS 系统架构 / 具身智能
- 🔧 熟悉：电机控制、运动学解算、轨迹规划、多机协同
- 🧠 研究：Imitation Learning + Reinforcement Learning for Manipulation
- 🌱 学习中：Renesas / Rust / Qt 深入开发
- ⚡ 兴趣：机器人系统设计、自动化设备、开源硬件

---

## 🚧 Featured Projects

| Project | Stack | What I Did |
|--------|-------|------------|
| 🦾 Tomato Picking Robot | STM32 + ROS + MoveIt + Vision | 机械臂底层驱动开发、运动学解算、ROS 控制链路 |
| 🤝 Dual-Arm IL + RL System | LeRobot + PyTorch + ROS2 | 机械臂模仿学习 + 强化学习策略训练与部署 |
| 🚗 Wheel-Rail Integrated Chassis | 电机驱动 + SDK 设计 | 舵轮控制、底盘运动控制算法、通信接口协议设计 |
| 🐦 Automatic Feed Changing for Pigeon | MCU、RTOS | 从裸机到 RTOS 的重构 |

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Kaede-Rei&show_icons=true&theme=tokyonight&count_private=true" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Kaede-Rei&layout=compact&theme=tokyonight" width="48%" />
</div>

---

## 🧠 Current Focus

```text
✔ 机械臂控制栈（驱动 → MoveIt → IL-RL Policy）
✔ ROS2 系统工程化
✔ 嵌入式视觉 + 边缘计算
✔ Embodied AI for Real Robot
